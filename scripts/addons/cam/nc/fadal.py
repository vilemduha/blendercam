...
class Creator(iso.Creator):
    ...
    def program_begin(self, program_id, name=''):
        if self.use_this_program_id:
            program_id = self.use_this_program_id
        if self.PROGRAM() is not None:
            self.write('%')
            self.write('\n')
            self.writem([(self.PROGRAM() % program_id), self.SPACE(), (self.COMMENT(name))])
            self.write('\n')
    ...
    def program_end(self):
        if self.z_for_g53 is not None:
            self.write(
                self.SPACE() + self.MACHINE_COORDINATES() + self.SPACE() + 'Z' + self.fmt.string(self.z_for_g53) + '\n')
